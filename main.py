...
# Band-stop filter
def bandstop_filter(signal, fs, lowcut, highcut):
    nyquist = 0.5 * fs
    low = lowcut / nyquist
    high = highcut / nyquist
...
def generate_image(signal, filtered_signal):
    ...
    plt.subplot(2, 1, 1)
    plt.plot(signal)
    plt.title('Originalni EEG signal')
    plt.subplot(2, 1, 2)
    plt.plot(filtered_signal)
    plt.title('Filtrirani EEG signal')
    plt.tight_layout()
...
def separate_frequency_bands(signal, fs):
    # Apply Fourier transform to convert signal to frequency domain
    ...
    # Extract amplitude spectra for each frequency band
...
def generate_combined_plots(signal, filtered_signal, fs):
    ...
    delta_filtered_spectrum, theta_filtered_spectrum, alpha_filtered_spectrum, beta_filtered_spectrum, gamma_filtered_spectrum = separate_frequency_bands(filtered_signal, fs)
...
def main():
    ...
    eeg_signal = load_eeg_signal('C:/Users/User-PC/Desktop/dps/raw1.csv')
    ...
    if filter_choice == '1':
        ...
        filter_type = 'cheby1'  # Filter type (e.g., Butterworth)
        ...
        lowcut = 45.0  # Lower cutoff frequency
        highcut = 55.0  # Upper cutoff frequency
        fs = 512  # Sampling frequency (Hz)
        filtered_signal = bandstop_filter(eeg_signal, fs, lowcut, highcut)
    else:
